...
export default function Flashcards() {
  ...
  const displayWord = vocabWords.length > 0 ? vocabWords[currentIndex] : fallbackWord;
  ...
  return (
    <PageTransition className="bg-surface text-on-surface antialiased flex flex-col min-h-screen">
      ...
      <header className="bg-surface/90 backdrop-blur-md w-full top-0 sticky flex justify-between items-center px-6 py-4 z-50 border-b border-outline-variant/10 shadow-sm">
        <button onClick={() => navigate(-1)} className="text-primary hover:bg-primary/10 transition-colors p-2 rounded-full active:scale-95 duration-200 flex items-center justify-center">
          <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 0" }}>arrow_back</span>
        </button>
        <h1 className="font-headline font-bold text-primary text-xl text-center flex-grow tracking-tight">Vocabulary Flashcards</h1>
      ...
      <main className="flex-grow flex flex-col px-6 py-8 md:px-12 md:max-w-4xl md:mx-auto w-full relative z-10 pb-32">
        ...
        <div className="w-full mb-12">
          <div className="flex justify-between items-end mb-3 px-1">
            ...
            <span className="font-headline text-sm font-bold text-secondary">Card {currentCardNumber} of {totalWords}</span>
          </div>
          <div className="h-3 w-full bg-secondary-container rounded-full overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.05)]">
            <div className="h-full bg-secondary rounded-full shadow-[0_2px_4px_rgba(57,104,84,0.3)] transition-all duration-500 ease-out" style={{ width: `${progressPercent}%` }}></div>
          </div>
        </div>
        ...
        <div className="flex-grow flex flex-col justify-center items-center relative perspective-[1000px]">
          ...
          <div className="w-full max-w-lg bg-surface-container-lowest rounded-[2.5rem] p-8 md:p-12 shadow-[0_20px_40px_-8px_rgba(53,47,69,0.1)] border border-outline-variant/20 relative min-h-[450px] flex flex-col justify-center items-center text-center transition-transform duration-500 hover:-translate-y-2">
            ...
            <div className="w-full flex flex-col items-center justify-center flex-1">
              <h2 className="font-headline font-extrabold text-[3.5rem] md:text-[4rem] text-primary leading-tight mb-4 tracking-tight capitalize">{displayWord}</h2>
            ...
            <div className="w-full mt-10 pt-4 flex justify-between gap-4 animate-[fadeInUp_0.5s_ease-out]">
              <button onClick={handleNext} className="flex-1 py-4 rounded-full bg-error-container/20 text-error-dim font-headline font-bold text-lg hover:bg-error-container/40 transition-colors border border-error-container/30 active:scale-95">Hard</button>
              <button onClick={handleNext} className="flex-1 py-4 rounded-full bg-primary/10 text-primary font-headline font-bold text-lg hover:bg-primary/20 transition-colors border border-primary/20 active:scale-95 shadow-[0_4px_12px_rgba(44,100,133,0.1)]">Good</button>
              <button onClick={handleNext} className="flex-1 py-4 rounded-full bg-secondary-container text-on-secondary-container font-headline font-bold text-lg hover:bg-secondary transition-colors hover:text-on-secondary shadow-[0_4px_12px_rgba(57,104,84,0.2)] active:scale-95 border border-secondary/20">Easy</button>
            </div>
